...
export function useCapabilityTextToImage(): CapabilityTextToImage {
  ...
  const llmsModelServices = useModelsStore(({ sources }) => {
    const next = _findLlmsT2IServices(sources);
...
export function getActiveTextToImageProviderOrThrow() {
  ...
  const { sources: modelsServices } = llmsStoreState();
  const llmsModelServiceIDs = _findLlmsT2IServices(modelsServices);
...
function _findLlmsT2IServices(services: ReadonlyArray<DModelsService>) {
  return services
    .filter(s => s.vId === 'googleai')
  ...
  for (const { modelVendorId, modelServiceId, label, hasAnyModels } of llmsModelServices) {
    if (modelVendorId === 'googleai')
      providers.push({
        ...
        priority: 50,
        label,
        painter: 'Gemini Image Generation',
        description: 'Gemini Imagen models',
