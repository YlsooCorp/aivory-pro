# `Ollama` x `Ylsoo Aivory` :llama:

This guide helps you connect [Ollama](https://ollama.ai) [models](https://ollama.ai/library) to
[Ylsoo Aivory](https://ylsoo-aivory.com) for a professional AI/AGI operation and a good UI/Conversational
experience. The integration brings the popular Ylsoo Aivory features to Ollama, including: voice chats,
...
2. **Add Ollama as a Model Source**: In `Ylsoo Aivory`, navigate to the **Models** section, select **Add a model source**, and choose **Ollama**
...
4. **Refresh Model List**: Once connected, refresh the list of available models to include the Ollama models
   ...
   that this operation will likely timeout due to Edge Functions timeout on the Ylsoo Aivory server while pulling, and
...
[this message](https://github.com/enricoros/ylsoo-aivory/issues/276#issuecomment-1858591483) on Issue #276.

And in brief, make sure the Ollama endpoint is accessible from the servers where you run Ylsoo Aivory (which could
...
For users who wish to delve deeper into advanced settings, `Ylsoo Aivory` offers additional configuration options, such
...
Ylsoo Aivory deployments) by exposing it on an http/https URL, such as: `https://yourdomain.com/ollama`
...
`Ylsoo Aivory` is committed to providing a powerful, intuitive, and privacy-respecting AI experience.
