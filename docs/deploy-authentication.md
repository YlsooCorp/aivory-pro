...
`Ylsoo Aivory` does not come with built-in authentication. To secure your deployment, you can implement authentication
in one of the following ways:

1. Build `Ylsoo Aivory` with support for ⬇️ [HTTP Authentication](#http-authentication)
...
To enable it in `Ylsoo Aivory`, you **must manually build the application**:

- Build `Ylsoo Aivory` with HTTP authentication enabled:
...
- Configure the following [environment variables](environment-variables.md) before launching `Ylsoo Aivory`:
```dotenv
